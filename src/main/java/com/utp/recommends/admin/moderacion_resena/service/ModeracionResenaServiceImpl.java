...
public class ModeracionResenaServiceImpl implements ModeracionResenaService {
    ...
    @Override public List<ModeracionResenaResponse> pendientes() { return resenaRepository.findByEstadoOrderByFechaCreacionAsc(EstadoResena.PENDIENTE).stream().map(this::toResponse).toList(); }
    ...
    private ModeracionResenaResponse toResponse(Resena r) { return new ModeracionResenaResponse(r.getId(), r.getEstado().name(), r.getMotivoRechazo()); }
}
